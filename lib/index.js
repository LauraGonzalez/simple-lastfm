...
//changed getSessionKey method since the way it was doing the request is deprecated, now it should be a POST method under https
Lastfm.prototype.getSessionKey = function(callback) {
    var sig = 'api_key' + this.api_key + 'methodauth.getMobileSessionpassword' + this.password + 'username' + this.username + this.api_secret;
    var api_sig = md5(sig);
    var lastfmObj = this;
        ...
        function(error, response, body) {
            ...
            try {
                ...
                parser.parseString(
                    ...
                    if(ret.success) {
                        ret.session_key = result.lfm.session[0].key[0];
                        lastfmObj.session_key = result.lfm.session[0].key[0];
                    } else
                ...
                if(this.debug)
                    console.log("Exception getting session key: ", e);
            }
        });
};
